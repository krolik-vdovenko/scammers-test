...
if (currentTheme === 'light') {
    ...
    // Если нет сохранений, проверяем системные настройки
...
// ========== ЭФФЕКТЫ ПРИ ОТВЕТАХ ==========
// Создаём оверлей для вспышки (один раз)
...
function showFlash(color) {
    flashOverlay.classList.remove('red', 'green');
    void flashOverlay.offsetWidth; // форсируем рефлоу
...
function handleAnswer(answer) {
    ...
    // Эффекты при ответе
    if (delta > 0) {
        // Неправильный ответ (риск растёт)
        shakeBackground();
        showFlash('red');
    } else if (delta < 0 && !answer.isDangerous) {
        // Правильный ответ (риск падает)
        showFlash('green');
    }
